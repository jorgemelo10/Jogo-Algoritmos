...
class InimigoDiagonal(Inimigo):
    ...
    def move(self, lista_tiros_inimigos):
        ...
        self.x = self.x + self.direcao_x * self.velocidade_x

        if self.x <= 0 or self.x >= 256 - self.larg:
            self.direcao_x = self.direcao_x * -1
    def desenha(self):
...
class InimigoAtirador(Inimigo):
    def __init__(self):
        super().__init__()
        self.pontos = 6
        self.intervalo_tiro = random.randint(40, 60)
        self.timer_tiro = self.intervalo_tiro
    def move(self, lista_tiros_inimigos):
        super().move(lista_tiros_inimigos)
        self.timer_tiro = self.timer_tiro - 1
...
class Jogo:
    ...
    def update(self):
        ...
        self.frames_partida = self.frames_partida + 1
        
        self.bg_y = self.bg_y + self.speed_y
        
        if self.bg_y >= 256:
            self.bg_y = self.bg_y - 256
        ...
        for tiro in self.cenario.tiros:
            for inimigo in self.cenario.inimigos:
                if inimigo.verificar_colisao(tiro):
                    self.pontuacao = self.pontuacao + inimigo.pontos
        ...
        if self.cenario.trishot_timer > 0:
            self.cenario.trishot_timer = self.cenario.trishot_timer - 1
